...
const PLATFORM_POOLS: Record<string, typeof LINKEDIN_POOL> = {
...
function analyzeIcpMatch(jobTitle: string, industry: string): string {
...
export const getBulkPreview = async (req: Request, res: Response) => {
  try {
    const countParam = parseInt(req.query.count as string || '20', 10);
    const count = Math.min(Math.max(countParam, 10), 50);
    const platformKey = (req.query.platform as string || 'linkedin').toLowerCase().replace(' ', '_');
...
export const runBulkImport = async (req: Request, res: Response) => {
  try {
